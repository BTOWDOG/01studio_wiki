...
- `水平翻转(fliplr)`：训练图片水平镜像翻转概率，可设置范围 0.0 - 1.0 。方向敏感类任务：箭头、交通标志、文字、有左右朝向的目标建议设置为0。
...
图表下方会提供用户标注和实际训练效果供参考。

![train](./img/train/train22_1.png)
...
点击下载后出现下面界面，可以选择扫码部署或者手动部署。

![train](./img/train/train24_1.png)

#### 扫码部署

确保CyberCAM设备已经联网，打开 `扫一扫` 应用。

![train](./img/train/train24_2.png)

扫码二维码

![train](./img/train/train24_3.png)

点击安装，安装的本质是将代码文件下载存放到 /app 根目录。生产APP。

![train](./img/train/train24_4.png)

安装后如下图。

![train](./img/train/train24_5.png)


#### 下载源文件手动部署

点击【下载源文件】，文件下载后解压。可以看到有一些文件:
...
- `app.txt` ：APP信息
- `run.sh` ：APP执行文件，默认执行摄像头代码，即  `demop-camera.py` 。
- `icon.jpg` ：APP图标
- `demo-picture.py` ：识别图片demo，会用到val.jpg
- `demo-camera.py` : 使用摄像头识别 
- `yolo11n_cls_320.kmodel` ：适用于CyberCAM的模型文件
- `val.jpg` ：用于测试图片demo的图片


整个解压后的文件夹拖动到CyberCAM的 /app 根目录即可。
...
在开发板工程文件夹下执行 
```bash
python demo-picture.py。
```

![train](./img/train/train27.png)

即可在目录下生产result.jpg图片，看到识别结果。

![train](./img/train/train27_1.png)

![train](./img/train/train27_2.png)
...
- `demo-picture.py` ：识别图片demo，会用到val.jpg
- `demo-camera.py` : 使用摄像头识别 
- `yolo11n_det_320.kmodel` ：适用于CyberCAM的模型文件
- `val.jpg` ：用于测试图片demo的图片
...
![train](./img/train/train60.png)

即可在目录下生产result.jpg图片，看到识别结果。

![train](./img/train/train61.png)

![train](./img/train/train62.png)
