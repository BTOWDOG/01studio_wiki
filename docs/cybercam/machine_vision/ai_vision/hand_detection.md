...
手掌检测模型已经预先训练好，存放在代码同一目录下，我们只需要加载模型文件，使用kpu库进行推理，并将返回的结果画图并显示即可。

具体编程思路如下：

```mermaid
graph TD
    导入kpu等相关模块  --> 加载相关模型  --> 获取摄像头图像 --> AI视觉推理查找图像中手掌 --> 画图指示 --> 获取摄像头图像;
```
...
if os.path.exists("./hand_det.kmodel"):
    model_path = "./hand_det.kmodel"

# 使用系统绝对路径（IDE运行调试）
elif os.path.exists("/data/app/hand-det/hand_det.kmodel"):
    model_path = "/data/app/hand-det/hand_det.kmodel"
...
![hand_detection](./img/hand_detection/hand_detection2.png)
