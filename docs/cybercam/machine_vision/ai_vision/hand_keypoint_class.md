...
手势分类检测模型已经预先训练好，存放在代码同一目录下，我们只需要加载模型文件，使用kpu库进行推理，并将返回的结果画图并显示即可。

具体编程思路如下：

```mermaid
graph TD
    导入kpu等相关模块  --> 加载相关模型  --> 获取摄像头图像 --> AI视觉推理查找图像中手掌关键点数据--> 判读手势  --> 画图指示 --> 获取摄像头图像;
```
...
![hand_keypoint_class](./img/hand_keypoint_class/hand_keypoint_class2.png)
