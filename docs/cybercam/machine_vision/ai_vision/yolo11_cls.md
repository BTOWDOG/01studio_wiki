...
YOLO11 CLS物体分类支持1000种物体识别。分类（yolo11 cls）与检测（yolo11 det）区别是指返回物体名称，不返回物体位置坐标。

## 实验目的

编程实现物体分类识别并将识别结果画图指示。

## 实验讲解

基于CyberCAM K230 yolo11分类模型已经预先训练好，存放在代码同一目录下，我们只需要加载模型文件，使用kpu库进行推理，并将返回的结果画图并显示即可。

例程基于YOLO11, 支持识别1000种物体。

具体编程思路如下：

```mermaid
graph TD
    导入kpu等相关模块  --> 加载相关模型 --> 获取摄像头图像 --> 进行物体分类识别并将结果画图 --> 获取摄像头图像;
```
...
if os.path.exists("./yolo11n-cls-224.kmodel"):
    model_path = "./yolo11n-cls-224.kmodel"

# 使用系统绝对路径（IDE运行调试）
elif os.path.exists("/data/app/yolo11-cls/yolo11n-cls-224.kmodel"):
    model_path = "/data/app/yolo11-cls/yolo11n-cls-224.kmodel"

else:
    raise FileNotFoundError("模型文件缺失，请检查当前路径与系统路径下的模型文件是否存在。")

model_size = 224 #模型尺寸
yolo = kpu.YOLO11_CLS(model_path, model_size) # 加载模型 
...
![object_detection](./img/yolo11_cls/yolo11_cls1.png)

识别结果：

![object_detection](./img/yolo11_cls/yolo11_cls2.png)