...
车牌识别相关模型已经预先训练好，存放在代码同一目录下，我们只需要加载模型文件，使用kpu库进行推理，并将返回的结果画图并显示即可。

具体编程思路如下：

```mermaid
graph TD
    导入KPU等相关模块  --> 加载相关模型 --> 获取摄像头图像 --> AI视觉推理查找图像中车牌 --> 识别车牌内容 --> 画图描绘 --> 获取摄像头图像;
```
...
![hand_detection](./img/license_det_rec/license_det_rec2.png)
