...
用户可以通过windows读取SD卡或在核桃派该目录下自定义多个sh文件，实现开机执行各种自定义指令内容。下面是一些应用案例：

## 开机自动连接WiFi

烧录镜像后假设我们没有网线，也没有USB TTL串口工具，就可以使用这个功能让核桃派自动连接wifi,再通过路由器或者wifi扫描工具获取核桃派IP地址，实现SSH远程终端。（核桃派系统出厂带wifh.sh文件）。

将镜像卡插入电脑，看到的U盘就对应核桃派Linux系统的`boot`目录，在`/boot/start`路径下创建一个文件，名称以.sh结尾，如wifi.sh。
...
然后就可以通过ssh远程终端无线登录核桃派：[SSH远程终端教程](../os_software/terminal.md#ssh远程终端)
...
将上面代码保存名称为 **led_blink.py** 文件，放在核桃派系统 /home/pi 目录下用于测试。
...
将SD卡插入CyberCAM开发板启动系统或重启开发板，即可看到CyberCAM启动后蓝灯闪烁，说明自动运行python代码成功。
