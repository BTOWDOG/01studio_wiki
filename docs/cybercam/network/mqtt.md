...
## 实验目的
通过编程让CyberCAM实现MQTT协议信息的发布和订阅（接收）。
...
运行上方发布者代码后可以看到MQTT网络助手接收到CyberCAM发来的信息：
...
运行订阅者代码，可以看到thonny下方终端打印接收到的数据（实际是CyberCAM开发板接收到的数据）。
...
通过本节我们了解了MQTT通信原理以及成功实现通信。这个实验的MQTT是连接到通讯猫服务器的，所以是支持远程数据传输的。也就是你可以在家使用CyberCAM订阅公司或者学校实验室里面的CyberCAM，实现一些传感器等数据传输。目前市面上大部分物联网云平台支持MQTT，原理大同小异。大家可以基于不同平台协议来开发，实现自己的物联网设备远程连接。