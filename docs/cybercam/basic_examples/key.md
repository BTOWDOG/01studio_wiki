...
CyberCAM板载一个按键，位于TYPE-C供电口旁边,有丝印'K'字母的就是：

![key1](./img/key/key1.png)  <br></br>

从CyberCAM 引脚图可以看到按键连接到K230 引脚IO21, 没按下时输入高电平（1），按下接地输出低电平（0）：

![key2](./img/key/key2.png) <br></br>
...
```mermaid
graph TD
    导入digitalio相关模块-->构建key和led对象-->检测按键是否被按下-->按下执行LED状态翻转-->检测按键是否被按下;
```
...
while True:
    ...
    if key.value == 0: #按键被按下
        time.sleep(0.01) #10ms延时消抖
        if key.value == 0: #按键被按下
            state = not state #LED状态翻转
            led.value = state #输出状态，点亮或熄灭板载LED蓝灯
...
除了使用板载按键和LED外，你也可以自己搭建电路通过GPIO排针引脚连接，注意修改代码中的GPIO引脚编号即可。

按键的本质也是GPIO操作，除了板载的LED和按键，你也可以通过杜邦线连接到CyberCAM其它GPIO（12P排针上）使用相关功能。
