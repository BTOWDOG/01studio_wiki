...
## 前言
相信大部分人开始学习嵌入式单片机编程都会从点亮LED开始，CyberCAM的学习也不例外，通过点亮第一个LED能让你对CyberCAM GPIO应用和编程方法有一定的认识，为以后的学习和更大型的程序打下基础，增加信心。
...
CyberCAM板载一个可编程LED，位于按键旁边：
![led1](./img/led/led1.png)

从CyberCAM 引脚图可以看到LED连接到K230 IO52引脚, 通过输出高电平点亮：
...
```
参数说明：
- `pin` 开发板引脚编号。例：board.LED, board.IO14
...
LED蓝灯被点亮，不过蓝灯默认做了系统启动指示灯（成功启动蓝灯亮），所以看不出效果。
...
除了使用板载LED外，你也可以自己搭建电路接入GPIO排针引脚，注意修改代码中的GPIO引脚编号即可。
