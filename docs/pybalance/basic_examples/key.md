...
pyBalance开发板上有3个按键，RST、KEY1和KEY2，RST顾名思义是复位用的，所以真正自带可以编程使用的就有2个功能按键。检测按键是否被按下的方式是上电后判断按键引脚的电平，当被按下时候引脚为低电平“0”。从原理图可以看到，按键信号标记为 KEY1。KEY1 是原理图中的信号网络名称，实际连接到 ESP32-S3 的 GPIO0。
