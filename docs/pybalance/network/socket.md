...
## 前言
上一节我们学习了如何通过MicroPython编程实现让pyBalance连接到无线路由器。这一节我们则来学习一下Socket通信实验。Socket几乎是整个互联网通信的基础。
...
日期：2026.8
作者：01Studio
说明：通过Socket编程实现pyBalance与电脑服务器助手建立TCP连接，相互收发数据。
'''
...
if WIFI_Connect():
    ...
    s=usocket.socket()
    addr=('192.168.1.168',10000) #服务器IP和端口
    s.connect(addr)
